...
CraftDialog::CraftDialog(Personazh* p, QWidget* parent, int imageMode)
    ...
    QLabel* hintLabel = new QLabel("Ctrl+S — сохранить карточку и картинку");
    ...
    connect(saveShortcut, &QShortcut::activated, this, [=]() {
        ...
        QString baseFilename = QFileDialog::getSaveFileName(
            ...
            QDir::homePath() + "/Desktop/Карточка_" + person->name,
            "All files (*.*)"
        );
        ...
        // Сохраняем текст
        QFile txtFile(baseFilename + ".txt");
        if (txtFile.open(QIODevice::WriteOnly)) {
            QString content;
            content += "=== КАРТОЧКА ПЕРСОНАЖА ===\n";
            content += "Имя: " + person->name + "\n";
            
            if (auto mag = dynamic_cast<MagP*>(person)) {
                content += "Тип: Маг\n";
                content += "Стихия: " + mag->element + "\n";
                content += "Мана: " + QString::number(mag->mana) + "\n";
            } else if (auto vrag = dynamic_cast<VragP*>(person)) {
                content += "Тип: Враг\n";
                content += "Редкость: " + vrag->rarity + "\n";
                content += "Урон: " + QString::number(vrag->damage) + "\n";
            }
            
            content += "ХП: " + QString::number(person->health) + "\n";
            content += "Броня (шлем/кираса/сапоги): " + person->armor.toString() + "\n";
            
            QByteArray utf8Data = content.toUtf8();
            utf8Data.prepend("\xEF\xBB\xBF");
            txtFile.write(utf8Data);
            txtFile.close();
        }
        
        // Сохраняем картинку
        if (!currentPixmap.isNull()) {
            QString imgFilename = baseFilename + ".png";
            currentPixmap.save(imgFilename, "PNG");
        }
        
        QMessageBox::information(this, "Успех", 
            QString("Файлы сохранены:\n%1.txt\n%1.png").arg(baseFilename));
    });
