...
CraftDialog::CraftDialog(Personazh* p, QWidget* parent, int imageMode)
    ...
    // Ctrl+S — сохранить карточку в файл (с диалогом выбора)
    ...
    connect(saveShortcut, &QShortcut::activated, this, [=]() {
        ...
        QString filename = QFileDialog::getSaveFileName(
            ...
            QDir::homePath() + "/Desktop/Карточка_" + person->name + ".txt",
        ...
        if (file.open(QIODevice::WriteOnly)) {
            ...
            out << "=== КАРТОЧКА ПЕРСОНАЖА ===" << endl;
            out << "Имя: " << person->name << endl;
            
            if (auto mag = dynamic_cast<MagP*>(person)) {
                out << "Тип: Маг" << endl;
                out << "Стихия: " << mag->element << endl;
                out << "Мана: " << mag->mana << endl;
            } else if (auto vrag = dynamic_cast<VragP*>(person)) {
                out << "Тип: Враг" << endl;
                out << "Редкость: " << vrag->rarity << endl;
                out << "Урон: " << vrag->damage << endl;
            }
            
            out << "ХП: " << person->health << endl;
            out << "Броня (шлем/кираса/сапоги): " << person->armor.toString() << endl;
