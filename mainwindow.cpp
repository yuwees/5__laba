...
void MainWindow::setupUI() {
    ...
    // Обычный стиль (без hover в CSS)
    table->setStyleSheet(
        "QTableWidget { gridline-color: #a0a0a0; }"
        "QTableWidget::item { border: 1px solid #a0a0a0; padding: 6px; }"
        "QTableWidget::item:selected { background-color: #a060d0; color: black; }"
    ...
    // ВАЖНО: Включаем отслеживание мыши
...
void MainWindow::highlightRow(int row) {
    ...
    QColor hoverColor("#c896ff"); // Фиолетовый
...
void MainWindow::clearRowHighlight(int row) {
    ...
    // Возвращаем стандартный цвет (чередование или белый)
