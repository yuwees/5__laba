...
void MainWindow::setupUI() {
    ...
    // Таблица: 5 столбцов (без Кода, броня объединена)
    table = new QTableWidget(0, 5);
    table->setHorizontalHeaderLabels({"Имя", "Стихия/Редкость", "Мана/Урон", "ХП", "Броня"});
    
    table->setColumnWidth(0, 120);  // Имя
    table->setColumnWidth(1, 130);  // Стихия/Редкость
    table->setColumnWidth(2, 100);  // Мана/Урон
    table->setColumnWidth(3, 80);   // ХП
    table->setColumnWidth(4, 150);  // Броня
    ...
    table->setStyleSheet(
        ...
        "QTableWidget::item:hover { background-color: #d0d0ff; }"
        "QTableWidget::item:selected { background-color: #a0a0ff; color: black; }"
    ...
    connect(table, &QTableWidget::cellClicked, this, &MainWindow::onTableRowSelected);
...
void MainWindow::onTableRowSelected(int row, int /*column*/) {
    if (row >= 0 && row < static_cast<int>(persons.size())) {
        ...
        CraftDialog* dlg = new CraftDialog(selectedPerson, this);
