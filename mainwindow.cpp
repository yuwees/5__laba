...
void MainWindow::setupUI() {
    ...
    // ВАЖНО: Стиль для ВСЕЙ СТРОКИ при наведении
    table->setStyleSheet(
    ...
    // Отключаем автоматические действия при наведении
    ...
    // ТОЛЬКО клик открывает карточку
...
void MainWindow::refreshTable() {
    ...
    for (size_t i = 0; i < persons.size(); ++i) {
        ...
        // ВАЖНО: Убираем выделение для каждой ячейки
        for (int col = 0; col < 5; ++col) {
            QTableWidgetItem* item = table->item(static_cast<int>(i), col);
            if (item) {
