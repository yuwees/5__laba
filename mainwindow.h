...
private slots:
    ...
    void onTableRowHovered(int row, int column);  // <- НОВЫЙ СЛОТ ДЛЯ НАВЕДЕНИЯ
...
private:
    ...
    int previousHoveredRow;  // <- ЗАПОМИНАЕМ ПРЕДЫДУЩУЮ СТРОКУ
    ...
    void highlightRow(int row);  // <- ВСПОМОГАТЕЛЬНЫЙ МЕТОД
};
